# A testing script for a free piston driver optimiser
# Liam Heffernan
# 19/01/24
...
def constraint_function(driver_dict):
    ...
    driver_dict['driver_fill_composition'] = {'He': float(driver_dict['percent_He']/100), 'Ar': float(1 - driver_dict['percent_He']/100)}
    ...
    print('T3s = ', state3s.get_gas_state().T)
    print('P3s = ', state3s.get_gas_state().p)
    print('a3s = ', state3s.get_gas_state().a)
    ####################################################################################################################
    ...
    state2 = shock_tube.get_shocked_state()
    print('P2 = ', state2.get_gas_state().p)
    print('T2 = ', state2.get_gas_state().T)
    print('a2 = ', state2.get_gas_state().a)

    state3 = shock_tube.get_unsteadily_expanded_state()
    print('state3 = ', state3)
    print('a3 = ', state3.get_gas_state().a)
    ####################################################################################################################
    # returning the constraint function value:
    ####################################################################################################################

    return shock_tube.vs - 2500
...
def objective_function(driver_condition_dict):
    ...
    fill_condition = {
        ...
        "T_drvr_0": 298.15,  # K
        "composition_drvr": {'He': float(driver_condition_dict['percent_He']/100), 'Ar': float(1 - driver_dict['percent_He']/100)},
...
creator.create("FitnessMax", base.Fitness, weights=(1.0, )) # creating the fitness object. the positive weight means it is being maximised
...
start = time.time()
print('Shock speed difference is = ', constraint_function(driver_dict))
#print('Partial derivative wrt prup is:', finite_difference_calculation(constraint_function, 'p4', driver_dict))
end = time.time()
