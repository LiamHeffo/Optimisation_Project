...
def constraint_function(x):
    ...
    compression_ratio = driver_dict["p4"] / driver_dict["driver_p"]
...
def objective_function(x):
    ...
    # print(driver_condition_dict)

    # running some heuristic tests:
    if driver_condition_dict['driver_p'] > driver_condition_dict['reservoir_p']:
        return 0

    compression_ratio = driver_condition_dict["p4"] / driver_condition_dict["driver_p"]
    if 5 < compression_ratio < 70:
        pass
    else:
        return 0
    ...
    try:
        ...
        print("inputs = ", driver_condition_dict)
        print("Number of sims completed: ", sim_counter)
        quit()
...
# Defining binary tournament algorithm
def binary_tournament(pop, P, **kwargs):
    ...
    for i in range(n_tournaments):
        ...
        def fitness_function(pop, i):
            return 0.5*pop[i].F[0]
        if pop[a].F < pop[b].F:
            S[i] = a
...
def sim(n_individuals, n_gen, sim_number):
    start = time.time()

    # obj=[lambda x: -1*objective_function(x), ]
    obj=[lambda x: constraint_function(x),
         lambda x: -1*objective_function(x)]

    n_var=5
    xl = np.array([80, 1e2, 1e4, 0.065, 1e3])
    xu = np.array([100, 1e7, 40e6, 0.085, 10e6])
    problem = FunctionalProblem(n_var, obj, xl=xl, xu=xu)

    # creating a tournament selection object
    selection = TournamentSelection(pressure=2, func_comp=binary_tournament)

    # Sampling using Latin Hypercube Sampling
    sampling = LHS()

    # Creating a crossover object
    crossover = UniformCrossover()

    # Creating a mutation object
    mutation = PolynomialMutation()

    # Instantiating the solver algorithm
    algorithm = GA(pop_size=n_individuals, sampling=sampling, selection=selection, crossover=crossover, mutation=mutation,
                   eliminate_duplicates=True)

    res = minimize(problem, algorithm, termination=('n_gen', n_gen), seed=1, verbose=True, save_history=True)
    X = res.X
    F = res.F

    end = time.time()

    optimum_fill_condition = {'percent_He' : X[0], 'driver_p' : X[1], 'p4' : X[2], 'D_throat' : X[3], 'reservoir_p': X[4]}
    string = 'Optimum Fill Conditions: \n\n'
    for key in optimum_fill_condition:
        string += key + ': ' + str(optimum_fill_condition[key]) + '\n'
    print(string)
    print('F =', F)
    print("Sim Time: ", end - start, 's')

    from Running_Metric import RunningMetricAnimation

    running = RunningMetricAnimation(delta_gen=n_gen/5,
                            n_plots=5,
                            key_press=True,
                            do_show=True, sim_num=sim_number)

    print('res.history length', len(res.history))
    for algorithm in res.history:
        running.update(algorithm) # how do I save to a file?

# sim_list = ((50, 100), (100, 200), (200, 100))
sim_list = ((10, 5), (5, 5), (200, 100))
...
for tup in sim_list:
    ...
    n_gen = tup[1]
    n_individuals = tup[0]
    sim(n_individuals, n_gen, sim_number)