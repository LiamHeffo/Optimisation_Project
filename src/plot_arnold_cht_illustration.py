...
def draw_constraints(ax):
    ...
    # Infeasible fills. EPS has no alpha, and two stacked 0.55 fills would
    # force pdftops to rasterise the region. Reproduce the same composite with
    # opaque fills: paint the *union* of the infeasible regions at one-layer
    # opacity, then the *overlap* (the doubly-infeasible corner, which the two
    # transparent fills rendered darker) at the two-layer opacity on top.
    fill_1 = _blend(COL_INFEAS_FILL, 0.55)
    fill_2 = _blend(COL_INFEAS_FILL, 0.55, bg=fill_1)
    ax.fill_between(xs, YLIM[0], np.maximum(y1, y2), color=fill_1,
                    zorder=0.5, linewidth=0)
    ax.fill_between(xs, YLIM[0], np.minimum(y1, y2), color=fill_2,
...
# Faded original.
draw_ellipse(ax, PARENT, C_ISO, edgecolor=_blend(COL_ELLIPSE_OLD, 0.5), lw=0.9)
# Updated.
...
OUT_EPS = "figures/arnold_cht_illustration.eps"
